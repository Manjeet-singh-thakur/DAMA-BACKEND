...
authModel.resetPasswordEmail = async function (body) {
  ...
  if (results && results.length > 0) {
    let currentHashedPassword = results[0].uc_password;
    console.log(currentHashedPassword, "db_current_password");
    console.log(body.password, "db_new_password");

    if (passwordHash.verify(body.password, currentHashedPassword)) {
      console.log("Passwords match, cannot use the same password.");
      deferred.resolve({ success: false, message: "The new password cannot be the same as the current password. Please choose another password." });
    } else {
      let hashedPassword = passwordHash.generate(body.password);
      let updateObj = {
        uc_password: hashedPassword,
      ...
      if (result) {
        ...
        deferred.resolve({ success: false, message: "Failed to update password." });
      }
    }
  } else {
...
authModel.userChangePassword = async function (body, userId) {
  try {
    ...
    let checkingUuid = { uc_uuid: userId };
    ...
    let currentHashedPassword = result[0].uc_password;
    
    // Check if the new password is the same as the current password
    if (passwordHash.verify(body.newPassword, currentHashedPassword)) {
      throw new Error("The new password cannot be the same as the current password. Please choose another password.");
    }
    
    // Hash the new password
    let newHashedPassword = passwordHash.generate(body.newPassword);
    console.log(newHashedPassword, "New Hashed Password");

    // Update the password in the database
    let passwordObj = { uc_password: newHashedPassword };
    let resultOne = await mongoHelper.updateData(checkingUuid, "users_credential", passwordObj);
