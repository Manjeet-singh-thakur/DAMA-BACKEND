...
mongoObj.updateDataOnee = async function (filter, update, tableName) {
  let deferred = q.defer();
  ...
  if (filter && update && tableName) {
      MongoClient.connect(url, options, function (err, db) {
          ...
          database.collection(tableName).updateOne(filter, { $set: update }, function (err, res) {
              ...
              console.log("Document updated successfully:", res);
              deferred.resolve({ success: true, result: res });
          });
      });
  } else {
      console.error("Invalid parameters for updateData function");
      deferred.resolve({ success: false, error: "Invalid parameters" });
  }
  return deferred.promise;
};
/**
    ...
    MongoClient.connect(url, options, function (err, db) {
      if (err) {
        ...
        let database = db.db(dbName),
          ...
          query = {};
        if (body.perPage) {
          recordsPerPage = parseInt( body.perPage);
        }
...
/**
 * This function is used to get calculated totalProfit data.
 * @param       :
 * @returns     :
 * @developer   :
 * @ModifiedBy  :
 */
mongoObj.getTotalProfitDetail = async function(dataObj){
   // Check if resourcepurchaseData is an array and has elements
   if (Array.isArray(dataObj) && dataObj.length > 0) {
    // Use reduce to sum up the r_amount values, ensuring they are numbers
    totalProfit = dataObj.reduce((total, current) => {
      return total + parseFloat(current.r_amount || 0); // Convert r_amount to a number
    }, 0);
