...
resourceModel.insertResource = async function (body, userId) {
  ...
  if (body) {
    ...
    // let duplicateValue = body.desc ? cleanText(body.desc) : "";
    ...
    let insertObj = {
      ...
      r_avgRating: 0, // Assuming r_avgRating should have a default value
      r_pdf: "",
      r_image: "",
      r_active: "1",
      r_deleted: "0",
...
resourceModel.editResource = async function (body, userId) {
  ...
  if (body) {
    ...
    if(body.isPaid == "FREE"){
      ...
      results = await mongoHelper.updateData(selectObj, "resource", editObj);
    }
    else{
      let editObj = {
        r_title: body.title,
