...
memberModel.createMember = async function (body) {
  ...
  try {
    ...
    var hashPassword = passwordHash.generate(body.password);
...
memberModel.editMember = async (body) => {
  try {
    ...
    // Check if body.module exists and is an array
    if (body.module && Array.isArray(body.module) && body.role) {
      body.module.forEach((mainModule, index) => {
        const subModules = body[`${mainModule.toLowerCase()}-action`];
        modulesWithSubModules.push({ mainModule, subModules });
      });
    ...
    console.log(error);
    // Handle error appropriately, e.g., throw or return false
    // return false;
  }
};
